...
class RandomHabitAdapter(
    ...
    inner class PagerViewHolder internal constructor(itemView: View) :
        ...
        private val countDownButton: Button = itemView.findViewById(R.id.pager_btn_open_count_down)


        fun bind(pageType: PageType, pageData: Habit) {
            titleTextView.text = pageData.title
            startTimeTextView.text = pageData.startTime
            priorityImageView.setImageResource(getPriorityIcon(pageType))
            minutesTextView.text = pageData.minutesFocus.toString()
            countDownButton.setOnClickListener { onClick(pageData) }
        }

        private fun getPriorityIcon(pageType: PageType): Int {
